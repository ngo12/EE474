...
unsigned long timePerPixel = 0;  // ms

void loop() {
//  Serial.println("Main Loop");
  Serial.println(timePerPixel);
...
int LPF_Data;
long LPF_Data_BL;
//float LPF_Beta = 0.08; // 0<Beta<1
float LPF_Beta = 0.15; // 0<Beta<1
//float LPF_Beta_BL = 0.01;
//
...
//    LPF_Data_BL = LPF_Data_BL - (LPF_Beta_BL * (LPF_Data_BL - adcValueCopy));
//    LPF_Data_BL = LPF_Data_BL - (2048);
//    EMA_S = (EMA_a*LPF_Data) + ((1-EMA_a)*EMA_S);
//    HPF_Data = LPF_Data - EMA_S;  
//    HPF_Data = HPF_Data * 20;
    
    myTimer = millis() - myTimer;
    if (myTimer >= timePerPixel) { // TODO change back to timePerPixel
      drawNewData();
      myTimer = millis();
    }
  } else {
...
void drawNewData() {
  ...
  y = map(adcValueCopy, 0, 4095, 0, screenHeight + 360);
  y = y - 150; // offset
