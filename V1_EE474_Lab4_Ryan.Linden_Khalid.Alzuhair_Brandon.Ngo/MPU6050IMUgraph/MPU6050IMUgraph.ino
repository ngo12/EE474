/* 
  ...
  Using a quaternion-based approach, 
  display a bar graph for the yaw, pitch, and roll
  of the MPU6050 using Sebastian Madgwick's IMU algorithm
  Moving average is used to smooth the read values
...
// Calculates moving average, with a window of n
int arrayAverage(int myArray[], int n){
  int s = 0; // sum of moving average 
  for (int i=0; i< n; i++)
  {
    s = s + myArray[i];
  }
...
// window size
const int n = MOVING_AVG_SIZE;
// keeps track of where to add to moving average
int counter = 0; 

// mapping values to fit the lcd
int yawMapped;
int pitchMapped;
int rollMapped;
...
{  
  ...
  // Draw zero line for reference, draw again later so it shows for longer
  tft.drawLine(135,0,135,320,ILI9341_WHITE);

  // Serial print and/or display at 0.5 s rate independent of data rates
  delt_t = millis() - count;
  if (delt_t > LCD_REFRESH) { // update LCD once per half-second independent of read rate
  ...
  // For more see http://en.wikipedia.org/wiki/Conversion_between_quaternions_and_Euler_angles which has additional links.
    ...
    // Map values to LCD size and put in our moving average array
    yawArray[counter] = map(yaw,-180,180,30,240);
    pitchArray[counter] = map(pitch,-180,180,30,240);
    rollArray[counter] = map(roll,-180,180,30,240);

    // Keeps track of place in moving average array
    if(counter >= n-1) {
     counter = 0;
    }
    ...
    // Draw bar graphs
    tft.fillRect(30,3,arrayAverage(yawArray,n),50,ILI9341_RED);
    tft.fillRect(30,56,arrayAverage(pitchArray,n),50,ILI9341_GREEN);
    tft.fillRect(30,109,arrayAverage(rollArray,n),50,ILI9341_BLUE);
