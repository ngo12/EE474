/* 
  ...
  move the 3d cube on the LCD screen with the MPU6050
...
//    tft.drawLine(Render[i].p0.x,Render[i].p0.y,Render[i].p1.x,Render[i].p1.y, *(uint16_t*)&PIX);
    tft.drawLine(Render[i].p0.x,Render[i].p0.y,Render[i].p1.x,Render[i].p1.y, ILI9341_YELLOW);
  }
  ...
  delay(1000); 
   ...
   else
   {
    ...
    while(1) ; // Loop forever if communication doesn't happen
      ...
      tft.fillScreen(ILI9341_BLACK); // Fills the screen with black 
}
...
//    if (delt_t > 50) { // update LCD once per half-second independent of read rate
  ...
  // For more see http://en.wikipedia.org/wiki/Conversion_between_quaternions_and_Euler_angles which has additional links.
    ...
    yawArray[countera] = map(yaw,-180,180,-180,180);
    pitchArray[countera] = map(pitch,-180,180,-180,180);
    rollArray[countera] = map(roll,-180,180,-180,180);
...
/***********************************************************************************************************************************/
    ...
    // get moving averages
    yawAvg = arrayAverage(yawArray,na);
    pitchAvg = arrayAverage(pitchArray,na);
    rollAvg = arrayAverage(rollArray,na);
    
    // control size of cube with potentiometer
    val = analogRead(2)/4;
    setCubeSize(val);
    ...
    // control angles with yaw pitch and roll
    Xan = -rollAvg;
    Yan = -pitchAvg;
    Zan = -yawAvg;
    ...
    tft.setCursor(10, 10);
    tft.setTextSize(2);
    tft.print("YAW: "); tft.print((int)Zan);
    tft.setTextColor(ILI9341_BLUE);  
    tft.print("  PITCH: "); tft.print((int)Yan);
    tft.setTextColor(ILI9341_GREEN);  
    tft.print("       ROLL: "); tft.print((int)Xan);
